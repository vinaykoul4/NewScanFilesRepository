...
BEGIN
	...
	BEGIN TRY
			...
			FROM #OPCO CO
			INNER JOIN DBO.SCAN_SEP_ScanData_RetailReporting  SD WITH (NOLOCK)
			...
			SET SS.ScanDataStatus = CASE WHEN SS.ScanIncentiveOptinStatus = 0	THEN 'S'		--Submitted Status,if stores submits ScanData and not opts in ScanData as well as for stores Opts Out of Scan Incentive Option
								         ELSE 
											CASE
												 WHEN SS.FirstSubmissionDate >= SC.ProcessingQtrStartDate THEN 'O'									--Onboarding Status,if stores submits ScanData and opts in ScanData. First file submission Date fall in Current Quarter.
